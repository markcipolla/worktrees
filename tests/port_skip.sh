...
# Wait for the listener to report ready before we run setup.
for _ in $(seq 1 50); do
  grep -q ready "$tmp/listener.out" 2>/dev/null && break
  sleep 0.1
done

result="$("$cli" setup "$work" --json)"
...
# Now occupy 41002 too. Force re-setup — it should roll onto 41003.
python3 -c '
import socket, time
...
result2="$("$cli" setup "$work" --json --force)"
...
# Occupy 41003 as well — the whole range is now busy, setup must fail.
python3 -c '
import socket, time
...
if "$cli" setup "$work" --json --force 2> "$tmp/err"; then
  echo "FAIL: expected setup to error when range is exhausted"
  exit 1
fi
